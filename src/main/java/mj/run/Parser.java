...
public class Parser {
  ...
  Expr loadExpr(Expr expr) {
	...
	Node  ASTMircoJava() {
		...
		prog.setMain(new CallStat(t.line, main));
	...
	void VarDecl() {
		...
		type = Type();
		Expect(3);
		var = tab.insert(Obj.Kind.Var, t.val, type);
		var.setLine(t.line);
		while (la.kind == 10) {
			Get();
			Expect(3);
	...
	Stat  Statement(boolean inLoop) {
		...
		case 3: {
			...
			if (StartOf(2)) {
				...
				lineOfOp = t.line;
				expr = Expr();
				if (!var.canBeAssignedTo()) { SemErr("cannot store to operand kind " + var.getObj().name); }
				if (op == Operator.ASSIGN) {
				 if (!expr.type.assignableTo(var.type)) { SemErr("incompatible types"); }
				}
				else {
				 if (var.type != Tab.intType || var.type != Tab.intType) { SemErr("operand(s) must be of type int"); }
				}
				switch (op) {
				 case ASSIGN:
				...
				param = ActPars(var);
				node = new CallStat(lineOfCall, var, param);
			} else if (la.kind == 17) {
				Get();
	...
	Expr  Factor() {
		...
		if (la.kind == 3) {
			...
			if (la.kind == 13) {
				...
				expr = new Call(lineOfCall, (Designator) expr, param);
			}
		} else if (la.kind == 1) {
			Get();
			...
			if (la.kind == 15) {
				...
				expr = new NewArray(lineOfNew, obj, expr);
				Expect(16);
			}
			else {
			 if (type.kind != Struct.Kind.Class) { SemErr("class type expected"); }
			 expr = new New(lineOfNew, obj);
			}
		} else if (la.kind == 13) {
			Get();
			expr = Expr();
